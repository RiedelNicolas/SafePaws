...
export const PhotosUploader = ({ folder }: { folder: string | null }) => {
  ...
  const fetchImages = async () => {
    const storageRef = ref(FirebaseStorage, email! + (folder!! ? "_" + folder + "/" : ""));
  ...
  const uploadPhoto = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await Promise.all(Array.from(event.target.files!).map(async file => {
      const path = email + (folder!! ? "_" + folder + "/" : "/") + Math.random().toString();
